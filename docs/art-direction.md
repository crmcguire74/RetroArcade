...
## Bayou Crossing artwork and modeled assets

Original image-generation prompts and outputs are preserved in `design/bayou-crossing/images/` and `design/bayou-crossing/prompts.json`. All images used the built-in image generation tool: four isolated frog/truck/log/turtle references, an overall art-direction concept, a moss-and-soil terrain texture and a distant woodland panorama. The concept is used on the cabinet; the soil and woodland images are packed into the environment GLB. References are not represented as generated meshes.

Blender source: `design/bayou-crossing/meshes/bayou-assets.blend`, `bayou-world.blend`, and `bayou-cabinet.blend`. The cabinet reuses the original arcade's native construction, with new materials, marquee and generated artwork. The playable world contains actual modeled road, river substrate, plants, trees, lanterns, lily pads, frog and movable hazards/platforms. A curved background plane extends the distant woodland with the generated panorama.

`model_bayou.py`, `refine_bayou.py`, `finalize_bayou_form.py`, and `review_bayou_assets.py` preserve authoring/review steps. `export_bayou.py` requires recorded Form approval, bakes normal maps and exports geometry tiers. `export_bayou_low_textures.py` produces reduced texture tiers. The separate `prepare_bayou_review.py` and `integrate_bayou.py` are one-time preparation scripts; do not rerun them over completed approval records or integrated code.
